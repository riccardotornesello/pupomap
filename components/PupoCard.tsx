...
export const PupoCard: React.FC<PupoCardProps> = ({ 
    ...
    return (
      <div className="bg-white rounded-xl shadow-md border border-stone-200 overflow-hidden hover:shadow-lg transition-shadow">
        <div className="h-48 overflow-hidden relative group">
            <img src={pupo.imageUrl} alt={pupo.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        ...
        <div className="p-4">
          ...
          <p className="text-stone-600 text-sm line-clamp-2 mb-4">{pupo.description}</p>
          <button
            onClick={onViewOnMap}
  ...
  // Modal version
  return (
    <div className="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div
      ...
      >
        <div className="relative h-64 shrink-0 bg-stone-200">
          <img src={pupo.imageUrl} alt={pupo.name} className="w-full h-full object-cover" />
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto">
          <h2 className="text-2xl font-bold text-stone-900 mb-2">{pupo.name}</h2>
          ...
          <div className="mt-8 p-4 bg-yellow-50 rounded-lg border border-yellow-100 flex gap-3">
            ...
            <p className="text-xs text-yellow-800">
                Tradizione: Questo pupo verrà acceso alla mezzanotte del 31 Dicembre.
            </p>
          </div>
        </div>
