...
import { Map, List, Navigation, LogIn, User as UserIcon, MessageCircle } from 'lucide-react';
import { PupoMap } from './components/PupoMap';
    ...
    <div className="h-screen w-screen flex flex-col bg-stone-50 overflow-hidden relative font-sans">
      ...
      <header className="bg-white border-b border-stone-200 h-16 flex items-center justify-between px-4 sm:px-6 z-20 shrink-0 shadow-sm">
        <div className="flex items-center gap-2">
            ...
            <div>
                <h1 className="text-lg font-bold text-stone-900 leading-tight">Pupi di Gallipoli</h1>
      ...
      <main className="flex-1 relative overflow-hidden">
        ...
        {/* Map View */}
        <div className={`absolute inset-0 transition-opacity duration-300 ${viewMode === ViewMode.MAP ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
            <PupoMap 
                locations={PUPI_DATA} 
        ...
        {/* List View */}
        <div className={`absolute inset-0 bg-stone-50 overflow-y-auto p-4 sm:p-6 transition-opacity duration-300 ${viewMode === ViewMode.LIST ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
             <div className="max-w-4xl mx-auto pb-20">
                <div className="flex justify-between items-end mb-6">
                  <div>
                    <h2 className="text-2xl font-bold text-stone-800">Classifica Pupi 🏆</h2>
                    <p className="text-stone-500 text-sm">Vota il tuo preferito per il Capodanno!</p>
                  </div>
                </div>
      ...
      {!isChatOpen && (
        <button
          ...
          className="fixed bottom-20 md:bottom-6 right-4 z-50 bg-red-600 hover:bg-red-700 text-white p-3 rounded-full shadow-lg transition-transform hover:scale-105 flex items-center gap-2"
          aria-label="Apri chat guida"
        >
          <MessageCircle className="w-6 h-6" />
          <span className="font-bold hidden md:inline">Guida Pupi</span>
        </button>
      )}
      <GuideChat isOpen={isChatOpen} onClose={() => setIsChatOpen(false)} />
